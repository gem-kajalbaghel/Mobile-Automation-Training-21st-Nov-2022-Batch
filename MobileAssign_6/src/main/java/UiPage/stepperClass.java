...
//                "elementId", ((RemoteWebElement) driver.findElement(Locators.)).getId(),
//                "direction", "left",
//                "percent", 0.95
//        ));
        ((JavascriptExecutor) driver).executeScript("mobile: dragGesture", ImmutableMap.of(
                "elementId", ((RemoteWebElement) driver.findElement(Locators.srcDot)).getId(),
                "endX", 818,
                "endY", 729
        ));
        Thread.sleep(2000);
//        String successmessage=textOf((WithLocator) driver.findElement(Locators.dropped));
        String successmessage=driver.findElement(Locators.dropped).getText();
        System.out.println(successmessage);
        try {
            Assert.assertEquals(successmessage, "Dropped!");
        }
        catch (Exception e)
        {
...
//        System.out.println( thenReturnElementList(Locators.listOfELems));
          ...
          try {
              String msg = driver.findElement(Locators.toast).getText();
